...
class NotificationRepositoryTest {
    ...
    @BeforeEach
    void setUp() {
        user = userRepository.findById(UserConstants.TEST_USER_ID)
    ...
    void findAllByReceiverAndCreatedAtIsAfterAndOrderByIdDesc() {
        ...
        List<Notification> notifications = notificationRepository
            .findAllByReceiverAndCreatedAtIsAfterOrderByIdDesc(user,
                NotificationConstants.TEST_NOTIFICATION_BEFORE_DATE_TIME);

        List<Long> notificationIds = notifications.stream()
            .map(BaseEntity::getId)
            .collect(Collectors.toList());

        Assertions.assertThat(notificationIds.get(0) > notificationIds.get(1)).isTrue();
        Assertions.assertThat(notificationIds.get(1) > notificationIds.get(2)).isTrue();
        Assertions.assertThat(notificationIds.get(2) > notificationIds.get(3)).isTrue();
        Assertions.assertThat(notificationIds.get(3) > notificationIds.get(4)).isTrue();
    }
}
